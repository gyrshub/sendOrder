<template>
	...
	<view class="my">
		...
		<view class="content boxshadow">
			<view class="tool">
				<view class="tool-item" v-for="(item,index) in toolList" :key="index" @click="clickTool(item)">
					<view class="iconSize">
...
<script>
	export default {
		...
		methods: {
			async getUserInfo(){
				let res = await this.$get('/housekeeper/getHousekeeper')
			...
			onloadImg(){
				uni.chooseImage({
					...
					success: async (res) => {
						...
						let upload = await this.$API.put('/housekeeper/updateHousekeeperAvatar',{
							avatar:result
						})
						if(upload.code == 200){
							this.getUserInfo()
